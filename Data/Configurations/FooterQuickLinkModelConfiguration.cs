...
{
    // Seeds a default set of Quick Links at the first migration: the pages a
    // visitor is likely to want — the same eight as in the hamburger menu, plus
    // Call for Papers.
    //
    // IconSvg is exactly the inner markup of the matching icon from
    // .mobile-nav-link in _Layout.cshtml, which keeps the hamburger menu and
    // the footer looking the same for the same page. The attributes inside use
    // single quotes so they do not collide with the double quotes of the C#
    // string literal.
    //
    // An administrator can delete, hide or edit any of them afterwards: this is
    // a starting point, not a hard-coded list. Their order on the site is random
    // (see _Layout.cshtml), which is why there is no DisplayOrder here.
