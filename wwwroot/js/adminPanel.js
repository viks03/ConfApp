...
// How long a toast stays depends on its kind. An error is 0, meaning it never
// disappears on its own: the administrator has to be able to read it and close
// it themselves, because the text often comes from the server and is long.
const TOAST_DURATIONS = { success: 3500, info: 4000, warning: 6000, error: 0 };

// More than three at once cannot be read, so the oldest leaves.
const TOAST_LIMIT = 3;
...
function dismissToast(toast) {
    ...
    // transitionend may never arrive — a background tab, reduced motion — so
    // there is a timeout behind it.
...
window.showToast = function showToast(message, type = 'success', duration) {
    ...
    // textContent, not innerHTML: the error messages come from the server and
    // may contain anything.
    toast.querySelector('.toast-msg').textContent = message;
    toast.querySelector('.toast-close').addEventListener('click', () => dismissToast(toast));
    container.appendChild(toast);

    // The container is column-reverse, so the first child is the oldest.
    ...
    if (ms > 0) {
        ...
        // While the pointer is over a toast, its timer waits.
...
// CAUTION: a new tab in Index.cshtml does NOT work until it is added here as
// well. activateTab() rejects anything outside this list and falls back to
// 'tab-registrations' — the button is there, clicking it does nothing, and the
// console says nothing either. Easy to miss.
const VALID_TABS = new Set([
    ...
    'tab-emails', 'tab-health', 'tab-paycontrol', 'tab-styles', 'tab-changelog', 'tab-downloads', 'tab-themes',
]);
...
function bindEditRegModal(trigger) {
    ...
    // [AD-02] Remembers the state the modal was opened with, so that the save
    // handler can tell a checkbox that was just cleared from one that was
    // already clear.
    const emailConfirmedEl = document.getElementById('regEmailConfirmed');
    if (emailConfirmedEl) emailConfirmedEl.dataset.wasChecked = String(d.confirmed === 'Verified');
...
function bindTicketModal(trigger) {
    ...
    // The numeric prices: these are what charging uses. Empty means the tier
    // is not payable.
    setVal('ticketRegPriceEur',   d['regEur'] || '');
    setVal('ticketPromoPriceEur', d['promoEur'] || '');
...
function bindFooterLinkModal(trigger) {
    ...
    // setVal() assigns .value directly and fires no 'input' event, so the
    // character counters (see initCharCounters) do not refresh themselves when
    // a row is opened for editing. They are refreshed explicitly here, or they
    // read "0 / 60" next to a field that is already full.
...
// No page reload; the switch takes effect immediately. If the server refuses,
// it snaps back, so that it never shows a state that was not saved.
// ── Crypto: clearing out the dead orders ─────────────────────────────────
// Deletes the expired ones only. A confirmed order is the record of a real
// payment and stays.
        ...
        input.addEventListener('change', async function () {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    // The switch goes back: the state was NOT saved.
...
// order for.
// Payment Control — eight switches on three levels. The script only saves the
// state and keeps the hierarchy visible; what /Payment actually offers is
// decided on the server.
//
//   level       data-gate-key        data-gate-level
//   master      all                  all
...
//
// Request:  POST ?handler=TogglePaymentGate  { key, enabled }
        ...
        input.addEventListener('change', async function () {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    // The state was NOT saved, so the switch goes back.
                    this.checked = !enabled;
                    showToast(`Error: ${result.message}`, 'error');
                }
            } catch {
...
function validateForm(bannerId, rules, fileId = null, isNew = false) {
    ...
    rules.forEach(rule => {
        ...
        // rule.required is checked FIRST, but only for emptiness itself. An
        // optional field that does have a value still goes through maxLength,
        // min, max and pattern — otherwise an optional field such as Footer
        // Content → Brand Tagline would never be validated at all once
...
function initSaveHandlers() {
    ...
    on('saveRegistrationBtn', 'click', async function () {
        ...
        // [AD-02] The label says only "Email address verified". Clearing that
        // checkbox, however, hands the account to the cleanup service, which
        // deletes every unconfirmed account older than 24 hours — along with
        // the uploaded paper and the verification document. Participants who
        // have paid are now excluded from that (see CleanupService), but for
        // everyone else it is irreversible, and nothing in the interface says
        // so.
        const confirmEl = document.getElementById('regEmailConfirmed');
        if (confirmEl && !confirmEl.checked && confirmEl.dataset.wasChecked === 'true') {
            const isPaid = document.getElementById('regPaymentStatus')?.value === 'Confirmed';
            const warning = isPaid
                ? 'Unchecking "Email address verified" marks this account as unverified.\n\n'
                  + 'This attendee has a confirmed payment, so automatic cleanup will skip them — '
                  + 'but they will have to verify their email again before they can log in.\n\n'
                  + 'Continue?'
                : 'Unchecking "Email address verified" marks this account as unverified.\n\n'
                  + 'System Cleanup permanently deletes unverified accounts registered more than '
                  + '24 hours ago, together with their uploaded paper and verification document. '
                  + 'This attendee has no confirmed payment, so they WILL be deleted on the next '
                  + 'cleanup cycle (runs hourly).\n\n'
                  + 'Continue?';
    ...
    // ── The live stream link of one session ──────────────────────────────────
        ...
        btn.addEventListener('click', async function () {
            ...
            try {
                const result = await postJson('?handler=SaveSessionLiveLink', buildFormData({
                    ...
                    liveLinkUrl: '' // empty clears the link
                }));
    ...
    on('saveFooterContentBtn', 'click', async function () {
        // EMAIL_RE mirrors the regular expression in
        // OnPostSaveFooterContentAsync (Index.cshtml.cs). They are kept in step
        // by hand: change one and the other has to follow.
...
function initCharCounters() {
    ...
    fields.forEach(id => {
        ...
        // Refreshed immediately on load: otherwise a field already filled from
        // the database — Footer Content, which is seeded — reads "0 / 100"
...
/** HTML-escape a string. A shorthand for ConfApp.escapeHtml (common.js) —
    едно екраниране за целия панел. Старата верига тук не екранираше `'`,
    тоест не беше безопасна в стойност на атрибут. */
function escHtml(str) {
    return window.ConfApp.escapeHtml(str);
}
...
// ─────────────────────────────────────────────────────────────────────────────
// THE LEGAL TEXTS — the Quill editors and saving them
// ─────────────────────────────────────────────────────────────────────────────
// One implementation for all four pages (Privacy, Terms, Cookie Notice, Cookie
// Policy). Each of the four used to have its own copy of the same ~55 lines,
// and two of the comments claimed to mirror initPrivacyEditor exactly while
// sitting above blocks whose toolbars had already diverged. The toolbar still
// differs — deliberately — but it is a parameter now, not a reason for a
// fourth copy.

// The full toolbar: headings, colour, alignment, lists, indentation,
// blockquote, link, image. For the long legal pages.
const RTE_TOOLBAR_FULL = [
    [{ header: [2, 3, false] }],
...
// The short toolbar: the banner text is a few sentences, where headings,
// colour and images would mean nothing.
const RTE_TOOLBAR_MINIMAL = [
    [{ header: [2, 3, false] }],
...
// The short one plus lists, for the /Cookies page.
const RTE_TOOLBAR_MINIMAL_LISTS = [
    [{ header: [2, 3, false] }],
    ['bold', 'italic', 'underline'],
    ['link'],
...
// The time of the last save, as shown in the line above the editors. This was
// written out four times over. The administrator's own clock IS local time, so
// there is no time zone conversion here — unlike the UTC values that come from
...
function initRichTextEditor(cfg) {
    const enContainer = document.getElementById(cfg.enId);
    const bgContainer = document.getElementById(cfg.bgId);
    if (!enContainer || !bgContainer || typeof Quill === 'undefined') return;

    const quillEn = new Quill(enContainer, { theme: 'snow', modules: { toolbar: cfg.toolbar } });
    const quillBg = new Quill(bgContainer, { theme: 'snow', modules: { toolbar: cfg.toolbar } });
    ...
    const seedEn = document.getElementById(cfg.seedEnId);
    const seedBg = document.getElementById(cfg.seedBgId);
    if (seedEn) quillEn.root.innerHTML = seedEn.value;
    if (seedBg) quillBg.root.innerHTML = seedBg.value;

    on(cfg.btnId, 'click', async function () {
        ...
        try {
            const result = await postJson('?handler=' + cfg.handler,
            ...
            if (result.success) {
                ...
                // staleness bug.
                const stampEl = document.getElementById(cfg.stampId);
                if (stampEl) stampEl.textContent = formatStamp(new Date());
            } else {
                showToast(`Error: ${result.message}`, 'error');
            }
        } catch {
            ...
            resetButton(this, cfg.buttonLabel);
        }
    });
}
...
function initPrivacyEditor() {
    initRichTextEditor({
        enId: 'privacyEditorEn', bgId: 'privacyEditorBg',
        seedEnId: 'privacyContentEnSeed', seedBgId: 'privacyContentBgSeed',
        btnId: 'savePrivacyPolicyBtn', handler: 'SavePrivacyPolicy',
        toolbar: RTE_TOOLBAR_FULL,
        stampId: 'privacyLastUpdatedText',
        successMsg: 'Privacy Policy updated — live on the public page now.',
        buttonLabel: 'Save Privacy Policy',
    });
}

function initTermsEditor() {
    initRichTextEditor({
        enId: 'termsEditorEn', bgId: 'termsEditorBg',
        seedEnId: 'termsContentEnSeed', seedBgId: 'termsContentBgSeed',
        btnId: 'saveTermsOfUseBtn', handler: 'SaveTermsOfUse',
        toolbar: RTE_TOOLBAR_FULL,
        stampId: 'termsLastUpdatedText',
        successMsg: 'Terms of Use updated — live on the public page now.',
        buttonLabel: 'Save Terms of Use',
    });
}

function initCookieNoticeEditor() {
    initRichTextEditor({
        enId: 'cookieNoticeEditorEn', bgId: 'cookieNoticeEditorBg',
        seedEnId: 'cookieNoticeContentEnSeed', seedBgId: 'cookieNoticeContentBgSeed',
        btnId: 'saveCookieNoticeBtn', handler: 'SaveCookieNotice',
        toolbar: RTE_TOOLBAR_MINIMAL,
        stampId: 'cookieNoticeLastUpdatedText',
        successMsg: 'Cookie banner text updated — live on the site now.',
        buttonLabel: 'Save Banner Text',
    });
}

function initCookiePolicyEditor() {
    initRichTextEditor({
        enId: 'cookiePolicyEditorEn', bgId: 'cookiePolicyEditorBg',
        seedEnId: 'cookiePolicyContentEnSeed', seedBgId: 'cookiePolicyContentBgSeed',
        btnId: 'saveCookiePolicyBtn', handler: 'SaveCookiePolicy',
        toolbar: RTE_TOOLBAR_MINIMAL_LISTS,
        stampId: 'cookiePolicyLastUpdatedText',
        successMsg: 'Cookie Policy page updated — live on /Cookies now.',
        buttonLabel: 'Save Cookie Policy Page',
    });
}
...
document.addEventListener('DOMContentLoaded', () => {
    // The hamburger menu comes entirely from scriptMain.js, which the shared
    // _Layout.cshtml loads once; it is not duplicated here.
