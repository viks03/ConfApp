...
function handleIbanDone() {
    ...
    // The button is hidden immediately: the request is not instant and a
    // second click would submit a second transfer.
    if (btn) btn.style.display = 'none';

    // The antiforgery token, required for any POST to a Razor Page.
    var token = '';
    ...
    .then(function (data) {
        if (!data) return;
        if (data.success) {
            // .visible is what the CSS keys the confirmation box on.
            if (box) {
...
// ══════════════════════════════════════════════════════════════════
// CRYPTO — LOCKING AND UNLOCKING AT THE ORDER LIMIT
// ══════════════════════════════════════════════════════════════════
...
function selectCrypto(el, currency, network, coinName) {
    ...
    fetch('/api/crypto/create-order', {
        ...
        body:    JSON.stringify({
            currency: currency,
            network:  network,
            // The tier whose page this request came from. Without it the
            // controller was fixed on Early Bird and the slug in the address
            // meant nothing. The server cross-checks it against the
            // participation form and refuses a mismatch.
            slug:     (window.TierData && window.TierData.slug) || null
        })
    })
    .then(function (res) {
...
// ══════════════════════════════════════════════════════════════════
// RESTORING THE ACTIVE ORDER ON PAGE LOAD
// ══════════════════════════════════════════════════════════════════
